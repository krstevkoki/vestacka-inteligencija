...
class Missionaries(Problem):
    ...
    def successor(self, state):
        successors = dict()
        levo_M, levo_K, strana, desno_M, desno_K = state

        for i in range(1, K + 1):
            for j in range(0, i + 1):
                if strana == "L":
                    Statenew = (levo_M - i, levo_K - j, "D", desno_M + i, desno_K + j)
                    if i + j <= K and valid_state(Statenew):
                        key = str(i) + "M_" + str(j) + "K"
                        successors[key] = Statenew

                    Statenew = (levo_M - j, levo_K - i, "D", desno_M + j, desno_K + i)
                    if i + j <= K and valid_state(Statenew):
                        key = str(j) + "M_" + str(i) + "K"
                        successors[key] = Statenew
                else:
                    Statenew = (levo_M + i, levo_K + j, "L", desno_M - i, desno_K - j)
                    if i + j <= K and valid_state(Statenew):
                        key = str(i) + "M_" + str(j) + "K"
                        successors[key] = Statenew

                    Statenew = (levo_M + j, levo_K + i, "L", desno_M - j, desno_K - i)
                    if i + j <= K and valid_state(Statenew):
                        key = str(j) + "M_" + str(i) + "K"
                        successors[key] = Statenew
    ...
    def h(self, node):
        total = N * 2
        transferred = node.state[3] + node.state[4]
...
def valid_state(state):
    ...
    if levo_M < 0 or levo_K < 0 or desno_M < 0 or desno_K < 0:
        return False
    if (levo_K > levo_M > 0) or (desno_K > desno_M > 0):
        return False
    return True
...
if __name__ == "__main__":
    ...
    initial_state = (N, N, "L", 0, 0)
    goal = (0, 0, "D", N, N)
    problem = Missionaries(initial_state, goal)
