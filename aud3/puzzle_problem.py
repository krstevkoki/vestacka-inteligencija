...
class Puzzle(Problem):
    ...
    def successor(self, state):
        successors = dict()
        empty_ind = str(state).index('*')

        # Up
        if empty_ind > 2:
            swap_ind = empty_ind - 3
            successors["Up"] = str(state)[0:swap_ind] + "*" + str(state)[swap_ind + 1:empty_ind] + \
                               str(state)[swap_ind] + str(state)[empty_ind + 1:]

        # Down
        if empty_ind < 6:
            swap_ind = empty_ind + 3
            successors["Down"] = str(state)[0:empty_ind] + str(state)[swap_ind] + str(state)[empty_ind + 1: swap_ind] + \
                                 "*" + str(state)[swap_ind + 1:]

        # Left
        if empty_ind % 3 > 0:
            swap_ind = empty_ind - 1
            successors["Left"] = str(state)[0:swap_ind] + "*" + str(state)[swap_ind] + str(state)[empty_ind + 1:]

        # Right
        if empty_ind % 3 < 2:
            swap_ind = empty_ind + 1
            successors["Right"] = str(state)[0:empty_ind] + str(state)[swap_ind] + "*" + str(state)[swap_ind + 1:]
    ...
    def h(self, node):
        matches = 0
        for (t1, t2) in zip(self.goal, node.state):
            if t1 != t2:
                matches += 1
        return matches
...
if __name__ == "__main__":
    ...
    goal = "*12345678"
    problem = Puzzle(initial_state, goal)
