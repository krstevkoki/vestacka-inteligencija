...
class WaterJugs(Problem):
    def __init__(self, capacities=(5, 5), initial=(5, 0), goal=(0, 1)):
    ...
    def successor(self, state):
        ...
        if J0 + 1 <= C0:
            Jnew = J0 + 1, J1
            successors["add 1l to jug 0"] = Jnew
        if J1 + 1 <= C1:
            Jnew = J0, J1 + 1
            successors["add 1l to jug 1"] = Jnew
        if J0 < C0 and J1 > 0:
            diff = min(C0 - J0, J1)
            Jnew = J0 + diff, J1 - diff
    ...
    def goal_test(self, state):
        goal = self.goal
        return (goal[0] == state[0] or goal[0] == '*') and \
               (goal[1] == state[1] or goal[1] == '*')
