...
def number_hops(node):
    subproblem = GraphProblem(node.state, Kraj, germany_map)
    answer = uniform_cost_search(subproblem).solve()[1:]
    return len(answer)
...
if __name__ == "__main__":
    ...
    germany_map = UndirectedGraph(dict(
        ...
        Nurnberg=dict(Munchen=167),
        Augsburg=dict(Munchen=84)
    ))

    problem = GraphProblem(Pocetok, Kraj, germany_map)

    cost = astar_search(problem).path_cost
    print(cost)
