    ...
    for i in range(min_links):
        for node in nodes:
            if len(g.get(node)) < min_links:
                ...
                neighbor = numpy.argmin(nodes, key=distance_to_node)
...
if __name__ == "__main__":
    ...
    ABdistance = distance(graph.locations["A"], graph.locations["B"])
    BIdistance = distance(graph.locations["B"], graph.locations["I"])
    BCdistance = distance(graph.locations["B"], graph.locations["C"])
    HIdistance = distance(graph.locations["H"], graph.locations["I"])
    IJdistance = distance(graph.locations["I"], graph.locations["J"])
    FCdistance = distance(graph.locations["F"], graph.locations["C"])
    GCdistance = distance(graph.locations["G"], graph.locations["C"])
    CDdistance = distance(graph.locations["C"], graph.locations["D"])
    FGdistance = distance(graph.locations["F"], graph.locations["G"])
    FJdistance = distance(graph.locations["F"], graph.locations["J"])
    KGdistance = distance(graph.locations["K"], graph.locations["G"])
    LKdistance = distance(graph.locations["L"], graph.locations["K"])
    DEdistance = distance(graph.locations["D"], graph.locations["E"])
    DLdistance = distance(graph.locations["D"], graph.locations["L"])
    LMdistance = distance(graph.locations["L"], graph.locations["M"])
    ...
    problem1_1 = GraphProblem(Pocetok, Stanica1, graph)
    problem1_2 = GraphProblem(Stanica1, Kraj, graph)

    problem2_1 = GraphProblem(Pocetok, Stanica2, graph)
    problem2_2 = GraphProblem(Stanica2, Kraj, graph)

    path_cost1 = astar_search(problem1_1).path_cost + astar_search(problem1_2).path_cost
    path_cost2 = astar_search(problem2_1).path_cost + astar_search(problem2_2).path_cost

    answer1 = astar_search(problem1_1).solve() + astar_search(problem1_2).solve()[1:]
    answer2 = astar_search(problem2_1).solve() + astar_search(problem2_2).solve()[1:]
