...
def minMaxNrom(training_data, testing_data):
    # vashiot kod tuka
    cols_training = list()  # list of tuples, each tuple is (min_col_value, max_col_value)
    for i in range(len(training_data[0]) - 1):
        col = [row[i] for row in training_data]
        cols_training.append((min(col), max(col)))

    new_data = [normalize_row(row, cols_training) for row in training_data]
    new_testing_data = [normalize_row(row, cols_training) for row in testing_data]
